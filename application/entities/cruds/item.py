...
from application.entities.database.models import Item as dbItem
from application.entities.serializers import ItemCreate
...
def create(db: Session, item: ItemCreate):
    instance = dbItem(name=item.name, price=item.price,
                      price_combo=item.price_combo)
    db.add(instance)
    db.commit()
    db.refresh(instance)
    return instance
